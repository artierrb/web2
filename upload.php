  ...
  <body>
    <?php
        ...
        $last_id = mysqli_insert_id($con);
            ...
            {
              $target = "img/imgdb/location/".date('dmYHis').str_replace(" ", "", basename($_FILES['location_image_file']['name'][$i]));
              $image = $_FILES['location_image_file']['name'][$i];
              $text = $_POST['location_image_info'][$i];
              $newfilename= date('dmYHis').str_replace(" ", "", basename($_FILES["location_image_file"]["name"][$i]));

              if(move_uploaded_file($_FILES["location_image_file"]["tmp_name"][$i],"img/imgdb/location/" . $newfilename))
              {
        ...
        //$last_id_product = mysqli_insert_id($con);
            ...
            {
              $target_product = "img/imgdb/product/".date('dmYHis').str_replace(" ", "", basename($_FILES['product_image_file']['name'][$i]));
              $image_product = $_FILES['product_image_file']['name'][$i];
              $text_product = $_POST['product_image_info'][$i];
              $newfilenamePD= date('dmYHis').str_replace(" ", "", basename($_FILES["product_image_file"]["name"][$i]));

              if(move_uploaded_file($_FILES["product_image_file"]["tmp_name"][$i],"img/imgdb/product/" . $newfilenamePD))
              {
        ...
        $query_product = mysqli_query($con,$sql_service);
            ...
            {
              $target_service = "img/imgdb/service/".date('dmYHis').str_replace(" ", "", basename($_FILES['service_image_file']['name'][$i]));
              $image_service = $_FILES['service_image_file']['name'][$i];
              $text_service = $_POST['service_image_info'][$i];
              $newfilenameSV= date('dmYHis').str_replace(" ", "", basename($_FILES["service_image_file"]["name"][$i]));


              if(move_uploaded_file($_FILES["service_image_file"]["tmp_name"][$i],"img/imgdb/service/" . $newfilenameSV))
              {
        ...
        $query_restaurant = mysqli_query($con,$sql_restaurant);
            ...
            {
              $target_restaurant = "img/imgdb/restaurant/".date('dmYHis').str_replace(" ", "", basename($_FILES['restaurant_image_file']['name'][$i]));
              $image_restaurant = $_FILES['restaurant_image_file']['name'][$i];
              $text_restaurant = $_POST['restaurant_image_info'][$i];
              $newfilenameRT= date('dmYHis').str_replace(" ", "", basename($_FILES["restaurant_image_file"]["name"][$i]));


              if(move_uploaded_file($_FILES["restaurant_image_file"]["tmp_name"][$i],"img/imgdb/restaurant/" . $newfilenameRT))
              {
        ...
        $query_lodge = mysqli_query($con,$sql_lodge);
            ...
            {
              $target_lodge = "img/imgdb/lodge/".date('dmYHis').str_replace(" ", "", basename($_FILES['lodge_image_file']['name'][$i]));
              $image_lodge = $_FILES['lodge_image_file']['name'][$i];
              $text_lodge = $_POST['lodge_image_info'][$i];
              $newfilenameLD= date('dmYHis').str_replace(" ", "", basename($_FILES["lodge_image_file"]["name"][$i]));


              if(move_uploaded_file($_FILES["lodge_image_file"]["tmp_name"][$i],"img/imgdb/lodge/" . $newfilenameLD))
              {
        ...
        $query_festival = mysqli_query($con,$sql_festival);
            ...
            {
              $target_festival = "img/imgdb/festival/".date('dmYHis').str_replace(" ", "", basename($_FILES['festival_image_file']['name'][$i]));
              $image_festival = $_FILES['festival_image_file']['name'][$i];
              $text_festival = $_POST['festival_image_info'][$i];
              $newfilenameFT= date('dmYHis').str_replace(" ", "", basename($_FILES["festival_image_file"]["name"][$i]));


              if(move_uploaded_file($_FILES["festival_image_file"]["tmp_name"][$i],"img/imgdb/festival/" . $newfilenameFT))
              {
